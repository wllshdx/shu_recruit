    ...
    <script>
        $(document).ready(function () {
            ...
            $("#sendInvite").click(function(){
                var aa = pp+'sentInviteEmail.action';
                var idArr = [];
                $("#result input[type=checkbox]:checked").each(function(){
                    idArr.push($(this).val());
                });
                $("#sendInviteBtn").click(function(){
                    $("#subject").val($("#inviteSubject").val());
                    $("#message").val($("#inviteMessage").val());
                    $("#sendTo").val(idArr);
                    $("#send").attr("action",aa);
                    $("#send").submit();
                });
...
<div class="main-content">
    <div class="bar">
        <ul class="nav nav-pills">
            <li  class="active"><a href="${basePath}findPositionResume.action?currentPage=1&pid=${resume.pid}&state=w" target="_self">待处理</a></li>
    ...
    <!-- 发送邀请弹出框 begin -->
    <div class="modal fade" id="sendInviteModal" tabindex="-1" role="dialog" aria-labelledby="sendInvite">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title">发送邀请</h4>
          </div>
          <div class="modal-body">
            <form>
              <div class="form-group">
                <label for="recipient-name" class="control-label">邮箱标题:</label>
                <input type="text" class="form-control" id="inviteSubject">
              </div>
              <div class="form-group">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" data-dismiss="modal" id="sendInviteBtn">发送</button>
          </div>
        </div>
      </div>
    </div>
