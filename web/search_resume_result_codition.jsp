...
<body>
<form action="${basePath}findResume.action" method="post" class="form-horizontal form" enctype="multipart/form-data"
      name="form-condition" id="form-condition">
    ...
    <c:forEach items="${list}" var="list">
        <tr>
            ...
            <td width="400" class="basic">
                <div class="msg">
                    <p><a href="${pageContext.request.contextPath}/user/findMyResume.action?type=1&id=${list.uid}"
                          target="_blank">${list.name}
                        | ${list.sex}　|　${list.age}岁　|　${list.nativePlace}　|　${list.education}　|　${list.workYear} </a>
                    </p>
                    期待职位：<span>${list.positionApplied}</span> | 期待工作地点：<span>${list.exceptedCity}</span> |
                    期待月薪：<span>${list.exceptedSalary}</span>
                </div>
            </td>
            <td class="experience">
                <div class="msg">
...
<div class="page">
        <span class="left">
            ...
            <input type="button" value="转发到邮箱" class="btn btn-primary" id="sendToEmail" data-toggle="modal"
                   data-target="#sendToEmailModal">
            <input type="button" value="保存到电脑" class="btn btn-primary" id="sendToComputer">&nbsp;&nbsp;&nbsp; 
        </span>
    <span class="right">
            <a href="${basePath}findResume.action?page=1&keys=${keys}&positionApplied=${page.positionApplied}&education=${page.education}&workYear=${page.workYear}&exceptedCity=${page.exceptedCity}">首页</a>&nbsp;&nbsp;&nbsp;
            <a href="${basePath}findResume.action?page=${page.totalPage}&keys=${keys}&positionApplied=${page.positionApplied}&education=${page.education}&workYear=${page.workYear}&exceptedCity=${page.exceptedCity}">尾页</a>&nbsp;&nbsp;&nbsp;
...
<form action="" method="post" id="send">
    ...
    <input type="hidden" id="pid2" name="currentPage" value='${resume.currentPage}'>
